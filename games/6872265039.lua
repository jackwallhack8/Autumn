...
run(function()
	...
	local function buildHUD()
		...
		local si = sessioninfo.Objects
		if not si then return end

		local ox = PosX and PosX.Value or 8
		local oy = PosY and PosY.Value or 8
		local sz = TextSize and TextSize.Value or 14
		local textColor = Color3.fromHSV(
			ColorOpt and ColorOpt.Hue or 0,
			ColorOpt and ColorOpt.Sat or 0,
			ColorOpt and ColorOpt.Value or 1
		)

		local entries = {}
		for _, key in {'Kills', 'Beds', 'Wins', 'Games'} do
			local obj = si[key]
			if obj then
				table.insert(entries, key..': '..tostring(obj.Value))
			end
		end
		...
		for _, str in entries do
			temp.Text = str
			if temp.TextBounds.X > maxW then maxW = temp.TextBounds.X end
			lineH = temp.TextBounds.Y
		end
		temp:Remove()

		local padX, padY = 6, 5

		if BgToggle and BgToggle.Enabled then
			local bg = Drawing.new('Square')
			bg.Filled = true
			bg.Color = Color3.new()
			bg.Transparency = BgOpacity and (1 - BgOpacity.Value) or 0.5
			bg.Size = Vector2.new(maxW + padX * 2, lineH * #entries + padY * 2)
			bg.Position = Vector2.new(ox, oy)
			bg.ZIndex = 1
			table.insert(drawings, bg)
		end

		for i, str in entries do
			local t = Drawing.new('Text')
			t.Font = 0
			t.Size = sz
			t.Color = textColor
			t.Outline = true
			t.OutlineColor = Color3.new()
			t.ZIndex = 2
			t.Text = str
			t.Position = Vector2.new(ox + padX, oy + padY + (i - 1) * lineH)
			table.insert(drawings, t)
		end
	end

	StatsHUD = vape.Categories.Render:CreateModule({
		Name = 'StatsHUD',
		Tooltip = 'Shows live K/B/W/G on screen.',
		Function = function(callback)
			if callback then
				StatsHUD:Clean(runService.RenderStepped:Connect(buildHUD))
			else
				destroy()
			end
		end
	...
	PosX = StatsHUD:CreateSlider({
		Name = 'Position X',
		Min = 0,
		Max = 2000,
		Default = 8,
		Function = function() if StatsHUD.Enabled then buildHUD() end end
	})

	PosY = StatsHUD:CreateSlider({
		Name = 'Position Y',
		Min = 0,
		Max = 1200,
		Default = 8,
	...
	TextSize = StatsHUD:CreateSlider({
		Name = 'Text Size',
		Min = 10,
		Max = 24,
		Default = 14,
	...
	ColorOpt = StatsHUD:CreateColorSlider({
		Name = 'Color',
		DefaultValue = 0,
		DefaultSat = 0,
		DefaultOpacity = 1,
	...
	BgToggle = StatsHUD:CreateToggle({
		Name = 'Background',
		Default = true,
		Function = function() if StatsHUD.Enabled then buildHUD() end end
	})

	BgOpacity = StatsHUD:CreateSlider({
		Name = 'BG Opacity',
		Min = 0,
		Max = 1,
		Default = 0.5,
